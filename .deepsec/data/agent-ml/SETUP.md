# Agent setup for `agent-ml`

This is a deepsec scanning workspace. Project `agent-ml` was just registered
(target: `..`). Setup is incomplete — `data/agent-ml/INFO.md`
...
2. **Fill in `data/agent-ml/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id agent-ml
pnpm deepsec process --project-id agent-ml
```
