...
Currently configured project: `agent-ml` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/agent-ml/SETUP.md` to fill in
   `data/agent-ml/INFO.md`.
...
`--project-id agent-ml` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/agent-ml/`.
